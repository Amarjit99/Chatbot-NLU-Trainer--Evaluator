...
router.get('/workspaces', async (req, res) => {
  try {
    ...
    // Guard access to mongoose.connection.db in case DB is not yet connected
    const dbName = mongoose.connection && mongoose.connection.db ? mongoose.connection.db.databaseName : 'not connected';
...
// DELETE /api/admin/workspaces/:id - Delete a workspace and associated models (admin only)
router.delete('/workspaces/:id', adminAuth, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    await Model.deleteMany({ workspace: id }).catch(err => console.warn('Failed to delete models for workspace', id, err.message));
...
// PATCH /api/admin/workspaces/:id/status - Update workspace status (admin only)
router.patch('/workspaces/:id/status', adminAuth, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    res.json({ message: 'Workspace status updated', workspace: { id: workspace._id, status: workspace.status } });
  } catch (err) {
