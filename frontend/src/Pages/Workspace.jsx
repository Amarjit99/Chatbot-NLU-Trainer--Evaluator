...
export default function Workspace({ goToLogin, darkMode, setDarkMode }) {
  // Per-workspace icons were removed per user request; keep data shape but don't render icons
  ...
  const [confirmConfidence, setConfirmConfidence] = useState(null)
  const [showConfirmControls, setShowConfirmControls] = useState(false)
  const [showEditControls, setShowEditControls] = useState(false)
  const [editCorrectedIntent, setEditCorrectedIntent] = useState('')
  const [editConfidence, setEditConfidence] = useState(100)
  const [isSubmittingPredictionFeedback, setIsSubmittingPredictionFeedback] = useState(false)
        ...
        {workspaceCreationMode === 'new' && (
          <div className="new-workspace-form">
            ...
            <form onSubmit={createWorkspace} className="workspace-form">
              ...
              <button type="submit" className="create-btn" disabled={!newWs.trim()} aria-disabled={!newWs.trim()} title={newWs.trim() ? 'Create' : 'Enter a name to enable'}>
                <FiPlus size={16} />
                Create Workspace
              </button>
            </form>
          ...
          <ul className="ws-list">
            {workspaces.map(w => (
              <li key={w.id} className={`ws-item ${selectedWorkspace?.id === w.id ? 'selected' : ''}`}>
                ...
                <button
                  className="ws-remove"
                  onClick={(e) => { e.stopPropagation(); removeWorkspace(w.id, w.name); }}
                ...
                <div className="ws-intents">
                  ...
                  {predictionResult && (
                    <div className="prediction-result" style={{ marginTop: '1rem', padding: '1rem', background: '#f0f8ff', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                      ...
                      {/* User feedback controls for training */}
                      <div style={{ marginTop: '0.8rem', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                        <button
                          className="ws-button"
                          onClick={() => {
                            // open confirm controls
                            setConfirmConfidence(Math.round(predictionResult.confidence * 100));
                            setShowConfirmControls(true);
                            setShowEditControls(false);
                          }}
                          disabled={isSubmittingPredictionFeedback}
                        >
                          This is correct
                        </button>
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setShowConfirmControls(false);
                            setEditCorrectedIntent('');
                            setEditConfidence(Math.round(predictionResult.confidence * 100));
                          }}
                          disabled={isSubmittingPredictionFeedback}
                        >
                          Edit / Correct
                      ...
                      {showConfirmControls && (
                        <div style={{ marginTop: '0.6rem', padding: '0.6rem', border: '1px dashed #cbd5e1', borderRadius: '6px' }}>
                          <div style={{ marginBottom: '0.4rem' }}><strong>Confirm prediction confidence:</strong></div>
                          <input
                            type="range"
                            min="1"
                            max="100"
                            value={confirmConfidence ?? Math.round(predictionResult.confidence * 100)}
                            onChange={(e) => setConfirmConfidence(Number(e.target.value))}
                          />
                          <div style={{ marginTop: '0.4rem' }}>{confirmConfidence ?? Math.round(predictionResult.confidence * 100)}%</div>
                          <div style={{ marginTop: '0.6rem' }}>
                            <button
                              className="ws-button primary"
                              onClick={async () => {
                                try {
                                  setIsSubmittingPredictionFeedback(true);
                                  const token = getAuthToken();
                                  ...
                                  const payload = {
                                    workspaceId: selectedWorkspace.id,
                                    feedback: [ {
                                      sampleId: `interactive_${selectedWorkspace.id}_${Date.now()}`,
                                      text: utterance,
                                      correctedIntent: predictionResult.predictedIntent,
                                      originalPrediction: predictionResult.predictedIntent,
                                      originalConfidence: predictionResult.confidence,
                                      confidencePercent: confirmConfidence ?? Math.round(predictionResult.confidence * 100),
                                      feedbackType: 'confirmation'
                                    } ]
                                  };

                                  await axios.post('http://localhost:3001/api/active-learning/feedback', payload, { headers });
                                  // Immediately trigger retrain so user sees updated model
                                  try {
                                    await axios.post('http://localhost:3001/api/active-learning/retrain', { workspaceId: selectedWorkspace.id, includeOriginalData: true }, { headers });
                                    alert('Confirmation submitted and model retrained');
                                  } catch (rtErr) {
                                    console.warn('Retrain failed or queued, continuing:', rtErr?.response?.data || rtErr.message || rtErr);
                                    alert('Confirmation submitted; retrain queued in background');
                                  }
                                  setShowConfirmControls(false);
                                  setPredictionResult(null);
                                } catch (err) {
                                  console.error('Failed to submit confirmation feedback:', err);
                                  alert('Failed to submit confirmation feedback');
                                } finally {
                                  setIsSubmittingPredictionFeedback(false);
                                }
                              }}
                              disabled={isSubmittingPredictionFeedback}
                            >
                      ...
                      {showEditControls && (
                        <div style={{ marginTop: '0.6rem', padding: '0.6rem', border: '1px dashed #cbd5e1', borderRadius: '6px' }}>
                          <div style={{ marginBottom: '0.4rem' }}><strong>Correct the intent:</strong></div>
                          <input
                            type="text"
                            placeholder="Enter correct intent..."
                            value={editCorrectedIntent}
                            onChange={(e) => setEditCorrectedIntent(e.target.value)}
                          />
                          <div style={{ marginTop: '0.4rem' }}><strong>Confidence (%):</strong></div>
                          <input
                            type="range"
                            min="1"
                            max="100"
                          ...
                          />
                          <div style={{ marginTop: '0.4rem' }}>{editConfidence}%</div>
                          <div style={{ marginTop: '0.6rem' }}>
                            <button
                              className="ws-button primary"
                              onClick={async () => {
                                if (!editCorrectedIntent.trim()) {
                                  alert('Please enter the corrected intent');
                                  return;
                                }
                                try {
                                  setIsSubmittingPredictionFeedback(true);
                                  ...
                                  const payload = {
                                    ...
                                    feedback: [ {
                                      sampleId: `interactive_${selectedWorkspace.id}_${Date.now()}`,
                                      text: utterance,
                                      correctedIntent: editCorrectedIntent.trim(),
                                      originalPrediction: predictionResult.predictedIntent,
                                      originalConfidence: predictionResult.confidence,
                                      confidencePercent: editConfidence,
                                      feedbackType: 'correction'
                                    } ]
                                  };

                                  await axios.post('http://localhost:3001/api/active-learning/feedback', payload, { headers });
                                  // Immediately retrain to apply correction
                                  try {
                                    await axios.post('http://localhost:3001/api/active-learning/retrain', { workspaceId: selectedWorkspace.id, includeOriginalData: true }, { headers });
                                    alert('Correction submitted and model retrained');
                                  } catch (rtErr) {
                                    console.warn('Retrain failed or queued, continuing:', rtErr?.response?.data || rtErr.message || rtErr);
                                    alert('Correction submitted; retrain queued in background');
                                  }
                                  setShowEditControls(false);
                                  setPredictionResult(null);
                                } catch (err) {
                                  console.error('Failed to submit correction feedback:', err);
                                  alert('Failed to submit correction feedback');
                                } finally {
                                  setIsSubmittingPredictionFeedback(false);
                                }
                              }}
