...
router.post('/feedback', auth, async (req, res) => {
  try {
    ...
    const validatedFeedback = feedback.map((item, index) => {
      ...
      // Normalize user-provided confidence (accept percent 1-100 or decimal 0-1)
      let userConfidence = undefined;
      if (typeof item.confidencePercent === 'number') {
        userConfidence = Math.max(0, Math.min(100, item.confidencePercent)) / 100;
      } else if (typeof item.userConfidence === 'number') {
        // assume 0-1
        userConfidence = Math.max(0, Math.min(1, item.userConfidence));
      } else if (typeof item.confidence === 'number') {
        // accept decimal 0-1 or percent >1
        if (item.confidence > 1) {
          userConfidence = Math.max(0, Math.min(100, item.confidence)) / 100;
        } else {
          userConfidence = Math.max(0, Math.min(1, item.confidence));
        }
      }
      ...
      return {
        ...
        userConfidence: userConfidence, // normalized 0..1 or undefined
    ...
    // Trigger background retraining using feedback data + original training data
    (async () => {
      try {
        const retrainingData = activeLearningService.generateRetrainingData(workspaceId);

        // Combine with original model training data if exists
        let combinedData = retrainingData;
        try {
          const originalModel = huggingfaceService.getModelInfo(workspaceId);
          if (originalModel && originalModel.trainingData && Array.isArray(originalModel.trainingData)) {
            combinedData = [...originalModel.trainingData, ...retrainingData];
          }
        } catch (err) {
          console.warn('Could not retrieve original training data for retrain:', err.message);
        }

        if (retrainingData.length > 0) {
          // Start retraining in background; do not block the API response
          huggingfaceService.trainModel(combinedData, workspaceId)
            .then(async (trainingResult) => {
              try {
                console.log(`Background retrain completed for workspace ${workspaceId}:`, trainingResult.modelId || trainingResult);

                // Persist model to DB similar to upload-and-train flow
                ...
                await Workspace.findByIdAndUpdate(workspaceId, { $inc: { datasetsCount: 1 } }).catch(() => {});

              } catch (dbErr) {
                console.error('Failed to persist background retrain model info:', dbErr.message || dbErr);
              }
            })
            .catch((trainingErr) => {
              console.error(`Background retrain failed for workspace ${workspaceId}:`, trainingErr.message || trainingErr);
            });
        } else {
          console.log(`No retraining data generated for workspace: ${workspaceId}`);
        }
      } catch (bgErr) {
        console.error('Error triggering background retrain:', bgErr.message || bgErr);
      }
    })();

    res.json({
      message: 'Feedback submitted successfully; retraining queued',
      workspaceId,
      ...result,
