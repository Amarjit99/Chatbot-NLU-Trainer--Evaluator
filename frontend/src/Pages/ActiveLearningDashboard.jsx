    ...
    const feedbackEntries = Object.entries(pendingFeedback)
      ...
      .map(([sampleId, feedback]) => {
        ...
        return {
          ...
          // include user supplied confidencePercent (1-100) if provided
          ...(typeof feedback.confidencePercent === 'number' ? { confidencePercent: feedback.confidencePercent } : {})
        };
      });
                  ...
                  <div className="feedback-controls">
                    ...
                    <div className="confidence-input">
                      ...
                      <input
                        ...
                        max="100"
                        step="1"
                        value={pendingFeedback[sample.id]?.confidencePercent ?? Math.round(sample.confidence * 100)}
                        onChange={(e) => handleFeedbackChange(sample.id, 'confidencePercent', Number(e.target.value))}
                      />
                      <div className="confidence-value">{pendingFeedback[sample.id]?.confidencePercent ?? Math.round(sample.confidence * 100)}%</div>
                    </div>
